Feature: Planned project lifecycle
  ...
  Scenario: Clone one unambiguous repository safely
    Given selected-provider metadata resolves "api" to a clean HTTPS URL on its configured authority without userinfo, query, or fragment
    ...
    And credentials remain process-scoped without URL or configuration persistence
    And the selected identity is applied only to the local repository

  @planned @LIFECYCLE-CLONE-001 @LIFECYCLE-SAFETY-001
  Scenario: Refuse an unsafe clone
    Given repository resolution is ambiguous or its origin is not authoritative, clean HTTPS on the selected configured authority
  ...
  Scenario: Create a minimum release in order
    ...
    And provider metadata supplies a clean HTTPS push URL matching the current repository identity, origin, provider, repository, and authority after URL rewriting
    ...
    And Colt creates the local tag, pushes it with process-scoped credentials to the validated explicit URL, and creates the provider release in order
  ...
  Scenario: Preserve partial release state
    Given the local release tag was created and pushed
    When provider release creation fails
    Then Colt does not force, overwrite, or roll back the tag or release
    And the completed tag state and failed step are reported
