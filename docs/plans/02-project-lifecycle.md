...
| 7 | Hostile Git-config rejection | `internal/`, `features/` | ✅ done |
...
- **Repository-local hardening** — a preflight scan rejects hostile repository-local execution, transport, proxy, credential-helper, and hook-root configuration before any tag mutation while permitting Colt's own credential helper.
- **Tests** — active lifecycle acceptance and focused native-Git isolation coverage, including hostile repository-configuration rejection.
...
- `LIFECYCLE-SECURITY-001` through `003` have active hostile-configuration and hook-isolation coverage.
