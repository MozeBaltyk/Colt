...
Milestone 1 is normative but only partially implemented; its absent behavior is explicitly tagged `@unimplemented`. Milestone 2 is partially implemented (list, clone, release wired). Milestones 3 through 6 are planned and their `@planned` scenarios do not claim current command or release availability.
