...
4. **M7, run/deploy:** Gitea and Forgejo deployment/lifecycle orchestration is implemented (`colt run <gitea|forgejo> <name> [--replace]` plus `status`/`stop`/`start`/`rm`), including advertised external HTTPS URLs. Destructive live acceptance remains gated and has not been run, so M7 is not yet production-validated. TLS termination, administrator/token creation, and provider onboarding remain external/manual as described in the [run/deploy spec](07-run-deploy.md). M7 is a prerequisite for M4 local-provider sync.
