...
1.  **M1, authentication and blank initialization:** configure and authenticate the currently supported providers (GitHub, GitLab, Gitea, Forgejo) with environment credentials, `auth.source`/`credential_id` references, concise live/offline status, deterministic credential resolution, and local and remote `colt init <project>` behavior from [shared core](00-core.md) and [project initialization](01-project-init.md). Gitea and Forgejo have real container-backed vertical tests. Persisted credentials, plaintext fallback, and logout remain normative but `@unimplemented`; browser/device authentication and provider-side revocation are optional and `@unimplemented`. Environment credentials are the production path today.
