...
- **Provider type:** the hosting platform family a provider speaks to (`github`, `gitlab`, `gitea`, `forgejo`).
- **Account:** the authenticated provider principal.
- **Namespace:** the repository owner in provider-native terms (for example a GitHub/Gitea/Forgejo user or organization, or a GitLab group or subgroup/full path). Namespace semantics differ per provider type and live inside provider adapters; common code MUST only treat a namespace as an opaque owner string.
...
|:---------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------|
| `CORE-ARCH-001`      | Hosting operations **MUST** use direct supported-provider HTTP APIs (currently GitHub, GitLab, Gitea, and Forgejo) and **MUST NOT** invoke provider-specific CLIs (such as `gh` or `glab`) or `curl`.                        | Architecture review.                     |
...
`--noninteractive` is a root/global flag inherited by every Colt command. A command is interactive only when standard input is a terminal and `--noninteractive` is absent. GitHub OAuth Device Flow is interactive authorization for this purpose even though approval occurs outside standard input.

| ID             | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                     | Verification                                                                    |
|:---------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------------------------------------------|
| `CORE-CLI-001` | In interactive mode, when a command is missing a required positional argument, option, or flag, Colt **MUST** prompt for the missing value instead of failing immediately. Colt **MUST NOT** prompt for optional input or input with a declared default; the command's native default applies unchanged. When multiple required values are missing, prompts **MUST** be emitted one at a time in stable command order: positional arguments first in declaration order, then required options and flags in declaration order. | [`interactive_input.feature`](../../features/interactive_input.feature)          |
| `CORE-CLI-002` | `--noninteractive` **MUST** disable every prompt and interactive authorization flow. Non-terminal standard input **MUST** behave as noninteractive even without the flag, so redirected input and automation cannot wait for a prompt or authorization flow. Missing required input in either case **MUST** fail without mutation with an actionable error that identifies each missing value in the same stable order used for prompts and says how to supply it.                             | [`interactive_input.feature`](../../features/interactive_input.feature)          |
| `CORE-CLI-003` | Explicitly supplied input **MUST** be validated as supplied. An explicit invalid value **MUST** fail and **MUST NOT** be discarded, defaulted, or replaced through prompting. Prompt text and resulting human-readable output **MUST** be deterministic for the same command and inputs. Secret entry **MUST** be non-echoing; reusable secret values **MUST NOT** appear in prompt output, normal output, or errors, consistent with `CORE-CREDENTIAL-002`.                                      | [`interactive_input.feature`](../../features/interactive_input.feature)          |
...
``` text
colt auth login <github|gitlab|gitea|forgejo> <alias> [--replace]
colt auth status [alias] [--repository <project>] [--offline]
colt auth logout <alias> [--revoke]
```

`<github|gitlab|gitea|forgejo>` enumerates the currently supported provider types.
...
| `CORE-PROVIDER-005` | `colt auth status [alias] [--repository <project>]` **MUST** show configured providers deterministically by alias when no alias is supplied, or only the exact configured provider selected by `alias`. By default it **MUST** perform a lightweight read-only provider API check using the resolved credential and report the authenticated account and API connection state. Without `--repository`, a concrete current `origin` matching the configured transport **MUST** be checked as defined by `CORE-GIT-011`. With `alias --repository project`, successful API authentication **MUST** be followed by authoritative repository lookup through that provider's common client abstraction and the selected metadata clone target **MUST** be checked. `--repository` without `alias` **MUST** fail before credential, provider, or Git reads with an actionable error. `--offline --repository` **MUST** likewise fail before those reads because an explicit repository probe contradicts offline operation. Plain `--offline` **MUST** inspect configuration only and **MUST NOT** read environment, secure-store, or credential-file secret values, nor invoke provider, Git, or SSH operations. No status mode may mutate provider, Git, or configuration state. | [`provider_configuration.feature`](../../features/provider_configuration.feature) and authentication output in [shared core](00-core.md) |
| `CORE-PROVIDER-006` | For interactive `colt auth login github <alias>` with an effective `stored` credential source, when no reusable persisted credential or environment token resolves, Colt MUST automatically start GitHub OAuth Device Flow, regardless of whether `stored` was explicit or inherited during `--replace` and regardless of whether the configured Git transport is SSH or HTTPS. Colt MUST embed its public GitHub OAuth app client ID; users MUST NOT need to configure, flag, or enter a client ID. Device Flow MUST communicate directly with GitHub HTTP APIs, display the authorization URL and user code, wait for approval, authenticate the resulting API token to identify the account, and persist it through the credential subsystem's secure-store-first / explicit-consent plaintext fallback. In noninteractive mode it MUST NOT start Device Flow and MUST instead fail with the actionable missing-credential error defined by `CORE-CLI-002`. Colt MUST NOT invoke `gh`, `glab`, `curl`, or another provider-specific CLI. Git transport remains independent: SSH keys MUST NOT be treated as provider API credentials. The authorization/device code is not a reusable credential and MAY be displayed. | [`provider_configuration.feature`](../../features/provider_configuration.feature) |
| `CORE-PROVIDER-007` | Provider-specific authentication implementations MUST remain behind the provider abstraction. The common contract covers authentication, credential persistence, credential retrieval, account identification, failure classification, and secret handling. Provider-specific protocol details (for example GitHub device flow vs GitLab token mechanism) MUST remain inside provider integrations; providers NEED NOT expose identical protocols. Gitea and Forgejo authorization stay inside their adapters. | Architecture review. |
...
Human-readable provider names **SHOULD** use the platform's own name (`GitHub`, `GitLab`, `Gitea`, `Forgejo`). Compact state markers **SHOULD** use:
...
Providers **MUST** be displayed deterministically by alias as compact sections. The default command performs the read-only provider API check defined by `CORE-PROVIDER-005`. An optional alias limits output and checks to that exact configured provider. `--repository <project>` requires an alias and deliberately tests that provider's configured Git transport even when the current directory has no origin or has a different origin. An unknown alias or invalid project name MUST fail actionably before any network or Git operation.
...
personal (default)
  ...
  Transport:   HTTPS · ✓ Git authentication/connectivity and read access confirmed (clone/fetch/pull); push/write not checked
```
...
A successful `Connection` state means only that Colt reached the configured provider API, the credentials were accepted, and the authenticated account was identified. Repository transport is reported independently and **MUST NOT** be inferred from API success.
...
If `colt auth status` runs inside a Colt-managed Git repository whose concrete `origin` matches a configured provider, Colt **MUST** perform an additional read-only Git access check using that origin:

``` text
Git access:  ✓ read access confirmed (clone/fetch/pull); push/write not checked
```
...
The authoritative transport probe **MUST** use bounded native `git ls-remote` against a validated credential-free target. This validates Git authentication/connectivity and the repository read access required by clone, fetch, and pull. It cannot safely establish push/write permission without mutation and output **MUST NOT** claim that it does. For HTTPS it MUST use the same transient Colt credential-helper path as clone/fetch so the resolved token is tested without placing it in arguments, URLs, output, or persistent Git configuration. For SSH it MUST use the configured SSH target and the user's existing SSH configuration, keys, agent, `BatchMode=yes`, and strict host-key checking without injecting provider API credentials. Colt MUST NOT use `ssh -T` as the authoritative check: provider-specific exit codes and greeting text are inconsistent, and SSH account authentication alone does not prove access to the repository. An implementation MAY mention `ssh -T` as a manual diagnostic after an SSH failure.

For `colt auth status <alias> --repository <project>`, Colt **MUST** first complete the provider API authentication check, then obtain repository metadata with `Client.Get` through the common provider abstraction used by GitHub, GitLab, Gitea, and Forgejo. It MUST select `CloneURL` for configured HTTPS or `SSHURL` for configured SSH and validate the exact configured authority, namespace, and project before invoking Git. Missing, malformed, mismatched, or unavailable metadata MUST produce `Transport: not checked` with a safe reason; Colt MUST NOT guess a self-hosted clone URL. API and transport labels and outcomes remain independent, so repository lookup or transport failure MUST NOT rewrite API success. The current-origin fallback remains available only when `--repository` is absent and its origin uses the provider's configured transport.

This check **MUST** remain distinct from provider API connectivity and **MUST NOT** push, mutate remotes, modify repository state, persist helper configuration, accept new host keys automatically, or alter SSH configuration or agents. It **MUST NOT** run when there is no concrete matching repository remote to validate; transport is then reported as `not checked`. Git access MUST NOT be claimed merely because provider API authentication succeeds, and tokens or SSH private-key material MUST NOT be exposed. API success with transport failure, and transport success with API failure, MUST be reported as independent outcomes.
...
| `CORE-CREDENTIAL-008` | `colt auth login` MUST accept `--credential env` or `--credential stored` to explicitly select the credential source. For an effective `stored` source, whether explicit or inherited from the existing alias during `--replace`, login MUST try the reusable persisted credential first, then the explicitly declared or conventional environment token. In interactive mode, if neither resolves for GitHub, Colt MUST automatically use the Device Flow defined by `CORE-PROVIDER-006`; an explicitly declared but empty `--token-env` does not suppress that fallback. Other providers MAY prompt for a token according to their adapter. Noninteractive behavior follows `CORE-CLI-002` and MUST NOT start Device Flow. Every resolved or newly authorized token MUST be validated for format and authenticated before storage. A newly obtained stored credential MUST use the persistence behavior in `CORE-CREDENTIAL-005`; environment credentials selected via `--credential env` are never persisted. | [`provider_configuration.feature`](../../features/provider_configuration.feature) |
...
| `CORE-GIT-011` | Live `colt auth status` **MUST** report Git transport separately from provider API connectivity. Without `--repository`, a concrete current `origin` matching a configured provider and its configured transport MUST be probed as a fallback. With `<alias> --repository <project>`, after API authentication succeeds Colt MUST obtain authoritative repository metadata through the common provider client, select `CloneURL` for HTTPS or `SSHURL` for SSH, validate exact authority/namespace/project, and run bounded, noninteractive, read-only native `git ls-remote`. HTTPS MUST resolve the token through Colt's transient credential helper; SSH MUST use the user's existing SSH configuration/key/agent with batch mode and strict host checking and no token. The probe validates Git authentication/connectivity and read access required by clone/fetch/pull, not push/write permission. It MUST NOT expose credentials, prompt, push, persist configuration, mutate local or remote state, or change the independent API result. Unavailable or unsafe metadata MUST be reported as transport `not checked` without guessing a URL. `--offline --repository` is rejected before reads; plain `--offline` skips all checks. | [`provider_configuration.feature`](../../features/provider_configuration.feature) |
