...
| `LIFECYCLE-CLONE-001` | `colt clone` **MUST** resolve the provider and repository unambiguously, use the authoritative HTTPS clone URL from selected-provider metadata, keep credentials process-scoped, and refuse an unrelated or non-empty destination. The effective clone target **MUST** remain within the selected workspace root. | [`project_lifecycle.feature`](../../features/project_lifecycle.feature) |
